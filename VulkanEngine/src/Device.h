...
namespace vkInit 
{
	...
	*/
	bool CheckSuitable(const vk::PhysicalDevice& device, const bool debug) 
	{
		if (debug) 
	...
	vk::PhysicalDevice ChoosePhysicalDevice(const vk::Instance& instance, const bool debug) 
	{
		...
		*/
		for (vk::PhysicalDevice device : availableDevices)
		{
			...
			if (CheckSuitable(device, debug)) return device;
		}
	...
	*/
	vk::Device CreateLogicalDevice(vk::PhysicalDevice physicalDevice, vk::SurfaceKHR surface, bool debug)
	{
		...
		*/
		...
		{
			queueCreateInfo.push_back({ vk::DeviceQueueCreateFlags(), queueFamilyIndex, 1, &queuePriority });
		}
	...
	*/
	std::array<vk::Queue, 2> GetQueues(vk::PhysicalDevice physicalDevice, vk::Device device, vk::SurfaceKHR surface, bool debug)
	{
		vkUtil::QueueFamilyIndices indices{ vkUtil::FindQueueFamilies(physicalDevice, surface, debug) };
