...
namespace vkInit 
{
	...
	*/
	...
	/**
	Make and return a main command buffer.
	\param inputChunk the required input info
	\param debug whether the system is running in debug mode
	\returns the main command buffer
*/
	vk::CommandBuffer CreateCommandBuffer(CommandBufferInputChunk inputChunk, bool debug) 
	{
	...
	/**
		Make a command buffer for each swapchain frame.
		\param inputChunk the required input info
		\param debug whether the system is running in debug mode
		\returns the main command buffer
	*/
	void CreateFrameCommandBuffers(CommandBufferInputChunk inputChunk, bool debug) 
	{
		vk::CommandBufferAllocateInfo allocInfo = {};
		allocInfo.commandPool = inputChunk.commandPool;
		allocInfo.level = vk::CommandBufferLevel::ePrimary;
